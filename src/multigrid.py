...
class MultigridSchedule(object):
    def init_multigrid(self, train_cfg, data_cfg):
        """
        ...
        train_cfg.multigrid.DEFAULT_B = train_cfg.batch_size
        train_cfg.multigrid.DEFAULT_T = cfg.DATA.NUM_FRAMES
        train_cfg.multigrid.DEFAULT_S = cfg.DATA.TRAIN_CROP_SIZE

        self.schedule = self.get_long_cycle_schedule(train_cfg, data_cfg)
    ...
    def update_long_cycle(self, train_cfg, data_cfg, cur_epoch):
        """
        ...
        if base_s != cfg.DATA.TRAIN_CROP_SIZE or base_t != cfg.DATA.NUM_FRAMES:
            ...
            print("\tBN.NORM_TYPE: {}".format(cfg.BN.NORM_TYPE))
    ...
    def get_long_cycle_schedule(self, train_cfg, data_cfg):
        """
...
class ShortCycleBatchSampler(Sampler):
    def __init__(self, sampler: Sampler, batch_size: int, drop_last: bool, train_cfg, data_cfg):
            ...
            int(
                round(
                    (
                            ...
                            / (s * train_cfg.multigrid.DEFAULT_S)
                    )
                    ** 2
                )
            )
            for s in train_cfg.multigrid.SHORT_CYCLE_FACTORS
        ]
